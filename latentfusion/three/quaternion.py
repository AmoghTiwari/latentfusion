...
def quat_to_mat(quaternion: torch.Tensor) -> torch.Tensor:
    ...
    Example:
        ...
        tensor([[[-1.,  0.,  0.],
                 ...
                 [ 0.,  0.,  1.]]])
    """
    quaternion, unsqueezed = core.ensure_batch_dim(quaternion, 1)
...
def mat_to_quat(rotation_matrix: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    Shape:
        ...
        - Output: :math:`(*, 4)`
    """
    rotation_matrix, unsqueezed = core.ensure_batch_dim(rotation_matrix, 2)
...
def qexp(q, eps=1e-8):
    """
    Computes the quaternion exponent.

    Reference:
        https://en.wikipedia.org/wiki/Quaternion#Exponential,_logarithm,_and_power_functions
    Args:
        q (tensor): the quaternion to compute the exponent of
    Returns:
...
def qlog(q, eps=1e-8):
    ...
    s, v = torch.split(q, (1, 3), dim=-1)
    w = torch.log(mag)
    xyz = (v / torch.norm(v, dim=-1, keepdim=True).clamp(min=eps)
           * core.acos_safe(s / mag.clamp(min=eps)))

    return torch.cat((w, xyz), dim=-1)


def qdelta(n, std, device=None):
    omega = torch.cat((torch.zeros(n, 1, device=device),
...
def perturb(q, std):
    ...
    """
    q, unsqueezed = core.ensure_batch_dim(q, num_dims=1)
